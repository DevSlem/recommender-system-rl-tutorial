...
import numpy as np
from gymnasium.spaces import Discrete, Dict, MultiDiscrete, Tuple
from collections import OrderedDict
from ray.rllib.utils.error import UnsupportedSpaceException
from ray.rllib.utils.spaces.space_utils import convert_element_to_space_type
from recsim_env.utils import gym_space_migration
...
class RecSimResetWrapper(gym.Wrapper):
    ...
    def __init__(self, env: gym.Env):
        ...
        self.observation_space = Dict(
            OrderedDict(
                [
                    ("user", gym_space_migration(self.observation_space["user"])),
                    ("doc", gym_space_migration(self.observation_space["doc"])),
                    ("response", Tuple([
                            Dict(OrderedDict(
                                [
...
class MultiDiscreteToDiscreteActionWrapper(gym.ActionWrapper):
    ...
    def __init__(self, env: gym.Env):
        ...
        if not isinstance(gym_space_migration(env.action_space), MultiDiscrete):
